<script lang="ts">
	...
	// Predefined pool of services not currently in the view
	...
	function toggleGroup(group: { name: string; components: string[] }) {
		const allChecked = group.components.every((c) => selectedComponents.includes(c));
		if (allChecked) {
			selectedComponents = selectedComponents.filter((c) => !group.components.includes(c));
		} else {
			const toAdd = group.components.filter((c) => !selectedComponents.includes(c));
...
<svelte:head>
	<title>Add Service | Statusy</title>
</svelte:head>

<div class="mx-auto w-3/5 pt-4 text-white">
	<!-- Header -->
	<div class="mb-6 max-w-xl">
		<h1 class="text-3xl font-extrabold tracking-tight text-white sm:text-4xl">Add Service</h1>
	...
	<div class="max-w-xl">
		<div class="grid gap-6">
				...
				<div class="relative">
					...
					>
						<option value="" disabled selected class="text-zinc-500"
							>Select a service to add...</option
						>
			...
			{#if selectedServiceId}
				{@const currentService = ALL_AVAILABLE_SERVICES.find((s) => s.id === selectedServiceId)}
				{#if currentService}
						...
						>
							<label class="group flex cursor-pointer items-start gap-3">
								...
								<div class="flex flex-col">
									<span
										class="text-lg text-sm font-medium text-zinc-200 transition-colors group-hover:text-white"
							...
							{#if componentMode === 'custom'}
								<div class="ml-7 grid max-h-72 grid-cols-1 gap-3 overflow-y-auto pr-1">
									{#each currentService.componentGroups as group}
		...
		<div class="mt-8 flex items-center justify-end gap-3 pt-5">
			...
			<Button
				...
				disabled={!selectedServiceId || selectedComponents.length === 0}
				onclick={saveService}
			>
				Add Service
			</Button>
