<script lang="ts">
	...
	$: deleteMutation = createMutation({
		mutationFn: (uuid: string) => _integrationAPI.DeleteSquadcast(uuid),
		onSuccess() {
			showModal = false;
			queryClient.invalidateQueries({ queryKey: [INCIDENT_MANAGEMENT_GET_QUERY_KEY] });
			_toast.success('Squadcast Integration is sucessfully deleted');
			webhookURL = '';
		}
	});
		...
		<label class="font-medium" for="webhook"
			>Your Webhook URL <span class="ml-1"
				>(<a
					...
					href="https://support.squadcast.com/integrations/incident-webhook-incident-webhook-api"
