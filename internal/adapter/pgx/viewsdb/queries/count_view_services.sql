SELECT 
  ...
  COUNT(CASE WHEN vs.monitor_incidents = true AND ai.id IS NOT NULL THEN 1 END) AS down_count,
  COUNT(CASE WHEN NOT (vs.monitor_incidents = true AND ai.id IS NOT NULL) THEN 1 END) AS up_count
