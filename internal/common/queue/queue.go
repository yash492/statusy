...
type Queue interface {
	...
	Send(ctx context.Context, queueName QueueName, payload json.RawMessage) (string, error)

	// SendBatch serializes multiple payloads to JSON and sends them as a batch.
	// Returns the message IDs of the enqueued messages.
	SendBatch(ctx context.Context, queueName QueueName, payloads []json.RawMessage) ([]string, error)

	// Read reads up to 'limit' messages with a visibility timeout of 'vt' seconds.
	Read(ctx context.Context, queueName QueueName, vt int, limit int) ([]Message, error)

	// Delete removes a message from the queue after successful processing.
	Delete(ctx context.Context, queueName QueueName, messageID string) error

	// Archive moves a message to the archive/dead-letter queue.
	Archive(ctx context.Context, queueName QueueName, messageID string) error
}
